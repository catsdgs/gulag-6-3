...
window.addEventListener('load', async()=>{
	...
	uptime_element.innerHTML = getTimeStr(new Date(stats.uptime * 1000 + (Date.now() - uptime_init)));
	...
	setInterval(()=>{
		uptime_element.innerHTML = getTimeStr(new Date(uptime_value * 1000 + (Date.now() - uptime_init)));
	}, 100);
});

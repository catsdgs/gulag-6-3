...
Element.prototype.setAttribute = function(){
	...
	if(value != null)switch(target.toLowerCase()){
		case 'href':
			if(!value.match(/^(?:blob|data|javascript):/gi)){ // dont do data url stuff
...
class ImageSpoof {
	constructor (){
		...
		img.addEventListener('loadstart', ()=>{
			...
			if(typeof vsrc == 'string' && !vsrc.startsWith(location.origin))img.src = proxifyURL(vsrc);
		});
	...
	switch(element_type.toLowerCase()){
		case'img':
			...
			element.addEventListener('loadstart', ()=>{
				...
				if(vsrc && !vsrc.startsWith(location.origin))element.src = proxifyURL(vsrc);
			});
...
window.Element.prototype.setAttribute = function(){
	...
	switch(target_class){
		case'src':
			var src = target_value;
			
			if(src != null){
				
				if(!src.startsWith(location.origin))src = proxifyURL(target_value);
...
Navigator.prototype.sendBeacon = function(url, data){
	...
	if(url.match(/^(?!data|blob|javascript).*/gi)){
		
		if(url.match(/^\/(?!https?:\/\/).*/gi)){ // value starts with / and not anythin else
			url = pm_url.origin + url
		}
		
		if(!url.startsWith(location.origin))url = location.origin + '/' + url // proxify url
	}
	// now that rewrites are done, get down to business
	
	return _Navigator_sendBeacon.apply(this, [url, data]);
}
